...
const probes = [
  ...
  ["switch", ".switch[disabled], .switch:disabled", ["backgroundColor", "borderColor"], "D2 switch disabled"],
  ["checkbox", ".checkbox", ["backgroundColor", "borderColor"], "D5 checkbox"],
...
for (const mode of ["light", "dark"]) {
  ...
  for (const [route, selector, props, label] of probes) {
    ...
    const rows = await page.evaluate(
      ({ selector, props }) => {
        ...
        const results = []
        for (const el of Array.from(document.querySelectorAll(selector)).slice(0, 3)) {
    ...
    for (const row of rows) out.push({ mode, label, route, ...row })
  }
  await ctx.close()
}
await browser.close()
...
for (const r of out) {
  ...
  seen.add(key)
  const flag = r.ratio < 3 ? "FAIL" : "ok  "
  console.log(`${flag} ${r.mode.padEnd(5)} ${r.ratio.toFixed(2).padStart(5)}:1  ${r.label} ${r.prop} ${r.value} on ${r.against}`)
}
