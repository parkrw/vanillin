...
function getOptions(listEl) {
  if (!listEl) return []
  return [...listEl.querySelectorAll('[role="option"]')].filter(
    (el) => !el.hasAttribute("aria-disabled") && !el.hidden
  )
}

/** cmdk's default filter is a fuzzy score that also re-sorts the list; ours
 * is a case-insensitive substring match (DOM order preserved) that also
 * searches the rendered label, so a slug `value` still matches its text. */
...
export function Command({
  ...
  const matches = useCallback(
    (itemValue, keywords, text) => {
      ...
      return fn(itemValue, search, extra) > 0
    },
    [shouldFilter, search]
  )
...
export function CommandList({ className, children, ...props }) {
  const { value, setValue, search, setEmpty, ids, listRef, listId, label } =
    useContext(CommandContext)
  ...
  }, [search, children, value, ids, listRef, setEmpty, setValue])
...
export function CommandItem({
  ...
  const visible = forceMount || matches(value, keywords, text)
