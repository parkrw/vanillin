 ...
 *   row identity     getRowId(item, index) keys row ids and selection;
 *                     defaults to the item's index in `data`
 ...
 */
// Module scope keeps the identity stable, so consumers that omit `getRowId`
// don't invalidate the row memo on every render.
const indexRowId = (item, index) => String(index)
...
export function useDataTable({
  ...
  // One pass per `data` identity instead of an indexOf scan per surviving
  // row: the scan was O(n^2) and re-ran whenever `filteredData` changed,
  // which an inline `columns` array makes every render.
  const dataIndex = useMemo(() => {
    const map = new Map()
    // First occurrence wins, matching the indexOf semantics this replaces.
    for (let i = 0; i < data.length; i++) {
      if (!map.has(data[i])) map.set(data[i], i)
    }
    return map
  }, [data])
  ...
  // `oi` is the position in `data` — it keeps the sort stable and survives
  // as the default row id. `rowId` is what selection keys on.
  const indexedRows = useMemo(
    () =>
      filteredData.map((item) => {
        const oi = dataIndex.get(item) ?? -1
        return { item, oi, rowId: getRowId(item, oi) }
      }),
    [filteredData, dataIndex, getRowId]
  )
  ...
  const grouped = useMemo(() => {
    ...
    if (activeGrouping.length === 0) {
      const leaves = sortRows(indexedRows).map(({ item, oi, rowId }) => ({
        kind: "data",
        item,
        oi,
        rowId,
    ...
    const build = (rows, depth, parentId) => {
      ...
      return entries.map(([value, bucket]) => {
        ...
        if (depth < activeGrouping.length - 1) {
          ...
          children = sortRows(bucket).map(({ item, oi, rowId }) => ({
            kind: "data",
            item,
            oi,
            rowId,
  ...
  }, [flatRows, pagination, maxPageIndex, manualPagination])

  // The slice above clamps for display only. Without writing the clamp back
  // to state, getCanPreviousPage() and the page indicator keep reporting a
  // page that no longer exists, so `previous` looks dead for several clicks.
  useEffect(() => {
    setPagination((p) =>
      p.pageIndex > maxPageIndex ? { ...p, pageIndex: maxPageIndex } : p
    )
  }, [maxPageIndex])
    ...
    const row = {
      ...
      toggleSelected(value) {
        setRowSelection((prev) => {
          const next = { ...prev }
          if (value ?? !prev[rowId]) next[rowId] = true
  ...
  function getIsAllPageRowsSelected() {
    ...
    return dataRows.every(({ rowId }) => !!rowSelection[rowId])
  }

  function getIsSomePageRowsSelected() {
    const dataRows = paginatedRows.filter((r) => r.kind === "data")
    if (dataRows.length === 0) return false
    const some = dataRows.some(({ rowId }) => !!rowSelection[rowId])
  ...
  const getCanNextPage = () => pagination.pageIndex < maxPageIndex
  ...
  function nextPage() {
    setPagination((p) =>
      p.pageIndex >= maxPageIndex ? p : { ...p, pageIndex: p.pageIndex + 1 }
    )
  }
