...
- **Gotchas:**
  - The panels have **no docs-site route**. They render from `site/showcase/panels/fixture.html`, which vite's dev server serves and the production build ignores (only `site/index.html` is a build input). If the supervisor gives them a real route, the fixture and its suite still work unchanged, but carry `window.__toast = toast` across with it: the suite dismisses toasts through that hook, and without it every dismissal silently waits out the 4s auto-dismiss timer (46s versus 5s for the suite).
