...
**Status:** COMPLETE
**Branch:** merged to main (`4ad8d502ed06`)  **PR:** none (local merge)  **Updated:** 2026-08-05

- **Landed:** Typeset rhythm system (3 vars + 3 font tokens), `styles/typeset.css` prose container with presets/opt-out, `ui/typography` retrofitted, inline `fontSize` replaced with `.pg-desc`/`.pg-detail` across ~18 pages, 11 new tests
- **Repo state:** clean on main
- **Next:** `/cycle 30` — docs-content, the final consistency/gap pass (absorbs ISSUES A3, B1–B6)
- **Gotchas:** `van.css` is still not imported by the docs site; typeset presets are baked into `typeset.css` as defaults
