...
- [x] 2. Demo page `site/pages/field.jsx` + registry `page:` entry — vertical/horizontal/responsive fieldsets, checkbox/switch rows, invalid + FieldError example. files: `site/pages/field.jsx`, `site/registry.js`
- [x] 3. RTL: sweep physical → logical properties (`ui/button-group/button-group.css` margin/corner radii; `ui/typography/typography.css` border/padding-left, text-align) + Direction demo page `site/pages/direction.jsx` (DirectionProvider dir="rtl" wrapping slider + horizontal field; breadcrumb dropped to stay under line cap) + registry entry — test: `tests/direction.test.mjs` proves arrows/pointer/thumb-position all mirror under `DirectionProvider dir="rtl"`; files: `tests/direction.test.mjs`, `site/pages/direction.jsx`, `site/registry.js`, two css files
