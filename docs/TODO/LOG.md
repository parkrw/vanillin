...
## Index history through 90 — moved out of README.md (2026-08-27, `/cycle --adjust`)

Verbatim copy of the README `**History:**` paragraph as it stood on 2026-08-27; per-task detail is also in each task file's `## Handoff`.
...
## Order wizard relayout — a quarter of tabs beside one card (2026-09-24)

Branch `feat/order-two-column`, reshaping PR #68 per issue #67. Left quarter: the steps as `ui/tabs` in two centred rows, the current step's options, one CTA foot. Right three quarters: one `ui/card` whose header holds the total (a region, not a card in a card) and the tools, then the vDC/VM table edge to edge, then the pager. Location is the site only — a users radio, the recommendation, site cards with DFW/CHI/SLC monograms on `ui/avatar`, Compare sites; billing and name moved to Review. Quick/Custom path, workload picker, header cart, price rail, mobile bar, on-page receipt and the Compact/Comfortable toggle are deleted; rows are compact only. No `ui/` edits.

- **Every override of a kit class wins on specificity, never on source order.** Arriving at `#order` from `#console` loads the order's kit sheets (`toggle-group`, `radio-group`, `pagination`, `menubar`, …) *after* `console.css`; a fresh `/order.html` loads them before it. A bare `.ck-users { display: grid }` therefore lost to `.toggle-group { display: inline-flex }` on one route only, and the users control ran under the table — which is why earlier QA on a fresh load missed it. Every such rule now carries the kit class (`.radio-group.ck-users`, `.card.ck-site`, `.input.ck-vm-input`, `.hover-card.ck-spec-card`, …). A CDP audit (`CSS.getMatchedStylesForNode`, every property where a `console.css` rule and a `ui/` rule tie on specificity for a rendered element) found 34 ties across 13 classes and now finds none; the suite walks both routes and compares the computed layout. Two kit-only ties surfaced on the way: `ui/pagination`'s Previous/Next padding against `ui/button`'s (pinned in `console.css`), and a consumer `display` at two classes against `.hover-card:not(:popover-open) { display: none }` — so `.ck-spec-card`'s grid is declared for `:popover-open` only.
- **Five labelled tab triggers do not fit the quarter.** Measured at 1280: 304px of column, ~390px of triggers at the kit's padding, 304px of label ink alone. Chosen with the user: two rows, `1 Location 2 Infrastructure 3 Storage` over a centred `4 Add-ons 5 Review`, plain digits instead of the circled number, 0.75rem labels. Montserrat is only named in the font stack, so the ink follows each machine's fallback; three labels have to fit row one under a wider fallback than the Mac's, and 0.75rem leaves ~10px there, ~30px here. `ui/tabs` cannot wrap as-is (`.tabs-trigger { height: calc(100% - 1px) }` of a one-row list paints row two under the panel); the trigger's block size is overridden, as the software tabs already do. Kit issue to file.
- **`@container ck-console` measures the whole frame, not the quarter** (~19rem at 1280), so every narrow-column rule is a base rule; the only order container query left is the 64rem stack. The card's own `vanillin-card` container stacks the total under the name below 34rem.
- **The vDC group row keeps to the columns.** One `colSpan` cell printed site, GHz, GB and storage under MACHINE while SITE/SIZE/IMAGE/COUNT stayed empty and POOL DRAW sat over the vDC's price. It is now one fact per visible header (`VDC_FACTS`, keyed by column id, so hiding a column drops its cell too), and the price column reads "Monthly" for both rows. Cell inline padding is 0.5rem: eight columns of selects and inputs have to fit three quarters of a 1280 frame without a sideways scroll.
- **The card fills the row.** A short table beside a long step left the three quarters bare; the section stretches, the table region grows, the pager holds the foot. Nothing is sticky (a sticky column taller than the viewport hides its own far edge).
- **The receipt cap is `min(70vh, 40rem)`.** Measured at the card's 24rem width, where each line's meta wraps under its label: 37rem default, 57rem with protection, backups and three add-ons. At 30rem every receipt scrolled and hid the total with tax.
- **`draft.workload` stays as a hidden default** ("general"): it still picks the new VM's size. Old exports carrying `path` import; `sanitizeVdc` drops it.

**Second pass, same day.** The steps went back to one strip across the top (`.tabs.ck-order-wizard` › `.tabs-list.ck-order-steps`: main's numbered circles and orange rule, kit classes in every selector); the two-row quarter is gone and the side column is a plain div. Location lost the users question and the recommendation line; the site cards keep the Recommended chip, read from `draft.users`, which stays a hidden default like `workload`. Infrastructure lost Software, Size and Compare all plans and is Network and Included. Access moved into `access-dialog.jsx`: Add VM (toolbar and row menu) opens it as "Add a machine to ‹vdc›" and saves the vDC's access with the machine; the access issue's link opens the same dialog as a fix, so `goTo(step, "access")` opens it instead of a step. It validates with `accessIssue()` before it closes, so no machine lands without a way in and a committed vDC never carries the problem.

- **The pools follow the machines.** With the sliders gone an overdraw had no control left to fix it, so `fitPools` grows CPU and RAM to the step that fits, never shrinking, on Add VM, count/size/image edits, Duplicate and import. The cpu/ram issues survive only past a pool's ceiling and say so. `draft.image` is a hidden default for the same reason, and `sanitizeVdc` checks an imported image against the catalogue.
- `COMPUTE_PRESETS`, `SOFTWARE_TABS` and `ORDER_COPY.headroom` had no consumer left and are gone.

**Third pass, same day.** The user wanted CPU and RAM back and the layout turned around: the options on the left three quarters, the price card on the right quarter, the table under both at the full width. Infrastructure is Size (the four plan cards and Custom, the CPU and RAM pool sliders, the draw line, Reserve headroom, Compare all plans), Network and Included again; `COMPUTE_PRESETS` and `ORDER_COPY.headroom` are back. `fitPools` stays: Add VM still grows a pool it would overdraw, and the slider is now the way to shrink one, so the cpu/ram issue reads "Grow the pool or shrink a machine" again unless the machines draw more than the pool's ceiling. `total-card.jsx` became `price-card.jsx`: `PriceCard` is the kit's card (`.card.ck-order-total-card`, kit class in the selector) holding the name and site, the live total with the receipt hover behind it, one button per billed group that sums the receipt's lines and jumps to the step that sets them, the sums, the whole order, and the billing note. It is `position: sticky` in the aside because it is short; the 64rem stack turns that off. The table card lost the total and gained a title with its counts. The foot is a row again: summary at the start, Continue or Deploy at the end at its natural width. Sections stay one column, hints capped at 72ch, and the grids take the width: sites three across, plans five, sliders and `.ck-order-two` two, BCDR three, all stacking under 48rem; the plans go three across under 72rem (three quarters of the frame is short of five), back to five once the body stacks, three under 48rem and two under 40rem.

**Fourth pass, 2026-09-25.** Infrastructure fits one 14-inch laptop screen (1512×860 viewport in the docs SPA, under its nav) on either size tab; the table is a scroll below, behind a `ui/separator` at the top of its section. Plans and Custom are `ui/tabs` in the Size head (four plan cards, then the pool sliders and headroom), the live draw line takes the place of the static hint, and Network sits beside Included (`.ck-order-two.ck-infra-pair`). The step lede, the header lede (no 72ch cap), the pool and IPv4 blurbs and the per-plan "estimate" were cut to fit; measured with Verdana forced as the sans, the foot still ends inside 860, so CI's fallback font should not break the fit test. A new vDC starts empty: `newOrder` and Add vDC add no machine, the last machine can be removed, and an empty vDC gets one placeholder leaf row (`placeholder: true`) so the table's own grouping, filter and paging carry it. Groups order by vDC id (`lib/use-data-table` sorts by value), not by where the placeholder sits. Access moved from the vDC to each machine (`vm.access`, `ACCESS_DEFAULTS`), with a username beside the password; Add VM still asks first and starts from the vDC's last machine, else the order's, and each machine's row menu carries "Access…". The access issue names its machine and opens that machine's dialog; `sanitizeVm` gives a machine from an older export its vDC's access. Only a vDC with machines needs storage.

**Fifth pass, 2026-09-25.** Six steps: Location, Infrastructure, Storage, Backup & DR (the explainer, protection, backups), Add-ons (edge services, licences), Checkout (billing and name, save and share, deploy). Each is an icon in a 1.75rem ring with an uppercase label under it and a 2px rule to the next; the steps behind the current one fill orange and so do their rules, the current one is ringed, the rest wait in grey. The rules are `aria-hidden` spans between the triggers inside the kit's `TabsList` (its roving focus selects `[role="tab"]`, so they are skipped). "Done" is positional, not validated: a flagged step still carries its count on the ring's shoulder. Continue left the foot for the strip's end, beside a Machines button that scrolls the table section into view and focuses it (`tabIndex={-1}`, no ring: the scroll is the feedback); the foot renders only with issues to list, or on Checkout, where it holds the deploy summary and Deploy. Protection and the add-ons gate on the three base steps' issues (`BASE_STEPS`), not on every other step's, so an unnamed vDC no longer locks the add-ons; `IssueGate` in `form.jsx` is the one alert for both and for Checkout's "Not ready to deploy".

- **The price card fills the column.** The body grid stretches (`align-items: stretch`), the aside is a flex column, the card `flex: 1` and its content `justify-content: space-between` over a 1rem gap: on a short step the card is the taller column and the blocks sit at the floor; on Backup & DR they spread evenly through ~1100px. Nothing is sticky any more.
- **The table is one element on every step.** It moved inside `.ck-order-body` after the aside; `grid-template-areas` puts it under both columns (`"main aside" "table table"`) and on Checkout beside the card (`"main aside" "table aside" "foot aside"`), with the foot rendered after it in the DOM on that step only, so reading order and layout agree and the table's grouping, filter and open rows survive the step change. The separator that closes the wizard hides on Checkout.
- **Compare sites is always open**, a `.ck-compare-block` with its own head under the cards: Site and the three latency columns. Tax stays on the card and the receipt; `queue` and `seismic` left `SITE_FACTS`. Plans keep their disclosure.
- Measured 1280, 1512×860 (Infrastructure still on one screen on either size tab; the strip grew ~16px, the foot's ~60px went), 700 (the actions wrap under the steps at the 64rem stack). Suite 22/22.

**Sixth pass, 2026-09-25.** The step heading is gone: the strip names the step, and each panel opens on its sections, whose hints absorbed the step ledes (`STEPS` carries id and label only). The strip sticks to the top of the scroll (`.ck-order-strip`, z-index 5, a `::before` bleeding the page background 1.25rem past both edges so a card's shadow at the column's edge never peeks past it); the price card is its own height again and sticks 0.75rem under the strip, whose measured height the page writes into `--ck-order-strip-h` from a `ResizeObserver` (the actions wrap under the steps at the stack, and a fixed figure would be wrong there); the same variable is the table section's `scroll-margin-block-start`, so Machines lands the table under the strip, not behind it. The table sits in the left column under the options on every step (`"main aside" "table aside" "foot aside"`, no Checkout-only areas), so the card runs beside both; the rule still closes the sections above it except on Checkout. The order page's own theme toggle is gone: the site nav has one. Continue precedes Machines at the strip's end. The table has a head like every section's ("vDCs and machines", the count as a chip, one line), and the card's toolbar lost its title.

- **Infrastructure** is Size, then Network beside Advanced networking, both always open (no disclosure): Network holds what costs, the IPv4 slider and the uplink; Advanced the default network and anti-affinity. Reserve headroom and anti-affinity are `CheckLine`s in `form.jsx`: the kit checkbox, a bold label, a muted clause, no card. `ORDER_COPY.headroom` is now that clause.
- **Add-ons** gained Included, beside Licences; the edge services are two across.
- **Storage** carries a note under the mix: the tier names (HDD, Hybrid, SSD, NVMe) are performance classes, not the hardware (`ORDER_COPY.media`, `.ck-order-note`).
- **Backup & DR**: the protection tiers are three cards across, the meta a line under the name (not a pill, which wrapped on Hot standby) and the delta on the card's last line; backups are the switch beside its retention and price line.
- **Compact pass**: section gaps 0.5→0.375rem, bodies 0.75→0.5rem, panels 1.25→1rem; option, check, switch and line rows at 0.5rem 0.75rem; plan and site cards, BCDR items, compare tables and presets tightened. Storage was the reference density and is unchanged.
- Measured 1440×900 and 1280×720 on every step; Infrastructure still fits 1512×860 on either size tab (the foot ends ~780px). Suite 22/22.

**Seventh pass, same day.** The strip sticks (`position: sticky` at the scroller's top, z-index 5, a `::before` bleeding the page background 1.25rem past both edges) and the price card sticks 0.75rem under it, at `calc(var(--ck-order-strip-h) + 0.75rem)`; the page measures the strip into that variable with a `ResizeObserver`, since the actions wrap under the steps at the stack. The same variable is the table section's `scroll-margin-block-start`, so Machines lands the table under the strip. The table sits in the left column under the options on every step (`"main aside" "table aside" "foot aside"`), the card beside both. The order page's own theme toggle is gone (the docs nav has one; the standalone `order.html` now has none).

- **Nothing is priced by the hour.** `HOURS_PER_MONTH`, `hourly` and `moneyHr` left `pricing.js` with every consumer: the card's hourly line, the receipt's hourly column and its 730-hour note, the plan cards (monthly is the price), the plan comparison's Per hour column, the vDC row's `/hr`, the deploy label ("Deploy — up to $x/mo"), the CSV's `hourly_usd` and the mail summary.
- **The card lists one line per billed step, always**: Compute, Network, Storage, Backup & DR, Add-ons, at zero until they bill, each a way to its step (`GROUPS` in `price-card.jsx` carries the step and field). The receipt's groups follow: the edge services and Windows licences are "Add-ons", replication and backups "Backup & DR". The card names the site alone; the vDC's name is the table's.
- **The total says it hovers**: a blue "Itemised receipt" cue under the figure, its chevron pointing left, where the card now opens (`side="left"`, `align="start"`), over the options, where a tall receipt has the room. `lib/anchor-position` clamps the card 8px inside the viewport on the cross axis and the cap is `min(calc(100vh - 1rem), 40rem)`, so the bottom is never cut off; the receipt scrolls inside.
- Site cards stack: the monogram over the name with the radio across from it. Figures in every table sit at the right, header included (`.ck-num` on the comparison headers, `data-col` on the order table's cells, `justify-content: flex-end` on the vDC price, which is a flex row). The latency headers read "Texas & the South", "Midwest & Northeast", "Mountain West & Pacific".
- Suite 22/22 on the order page; the full suite was 944/944 before the last batch and is rerun after it.
