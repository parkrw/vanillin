...
- **Suite baseline:** 834/834 on 2026-08-27 (`feat/console-glow`, idle machine, `node tests/run.mjs`, exit 0), no failing tests. That branch adds 3 panel tests, so `main` (`9dbd8eda8e04`) implies **831** — one below the 832 predicted after batch A, and not measured on `main` directly. Re-measure before every spawn and name the failing tests — a bare count is not evidence.
