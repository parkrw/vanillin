...
| 84  | token-guard              | ~S  | [ ]    | preventive — nothing broken today; build gate on undefined tokens [^84]; **+ ISSUES H4** (`tests/run.mjs` fails fast when its vite child dies — the busy-port refusal already landed in `0a3f51954733`); batch A, spawned 2026-08-27 |
