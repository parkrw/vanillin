...
**Resume:** **batch 4 is in flight (spawned 2026-08-13)** — three workers, one tmux window (`@7`): **81** in `../vanillin-task81` on `docs/layout-measure` (pane `%14`, test port 5201), **82** in `../vanillin-task82` on `docs/completeness` (pane `%15`, port 5202), **83** in `../vanillin-task83` on `fix/overlay-stacking` (pane `%16`, port 5203). All three branch from `3af9122`. Baseline re-measured on `main` at spawn time: **755/761** — six failures, not the three the last handoff recorded, so the extra three are being enumerated and are *not* worker regressions. `tests/run.mjs` pins `:5199` with `--strictPort`, hence the per-worker `VANILLIN_TEST_PORT`. **84 is the only unstarted task left after this batch, then 74 (deps 81), then the console kit.** Run `/cycle --adjust` to reconcile after the batch lands. Previous state: batch 3 complete and verified 2026-08-07 — 80 (759/761), 77c (757/759, 2 rework rounds), 77d (756/759, 1 rework round), all builds clean; the only failures are the two pre-existing slider-cursor tests plus an intermittent `navigation-menu` hover flake. **Task 77 is done — all 59 remaining component pages now carry the docs template.** *User merges four branches:* `fix/dialog-modal-positioning` (80), `docs/pages-rest-c` (77c), `docs/pages-rest-d` (77d), `docs/todo-batch3` (task files + this reconcile + ISSUES H3). No PRs — remote writes disabled. Worktrees `../vanillin-task{80,77c,77d}` stay until merged. **Next: 74's scope call** (is the docs site meant to work on a phone? — see [^74]), then the console kit.
...
| 81  | docs-layout-measure      | ~M  | [~]    | in flight (batch 4, `docs/layout-measure`) — `.pg-main` never centred; column measure + rhythm + topnav border/opacity [^81] |
| 82  | docs-completeness        | ~L  | [~]    | in flight (batch 4, `docs/completeness`) — 9 unfinished pages + home/hero + 353 em dashes + `docs/` reference [^82] |
| 83  | overlay-stacking         | ~S  | [~]    | in flight (batch 4, `fix/overlay-stacking`) — nav-menu viewport painted over by siblings; probe committed [^83] |
