...
**Resume:** Handoff section below — **batch A (84, 92, 93) merged 2026-08-27 (PRs #8, #9, #10)**; the head follow-ups (CopyField in the console, ringed live dots) are on `feat/console-glow`. Everything through **93** is merged; **74 is parked by the user** ("no mobile yet", 2026-08-24). **Next: phase 3 first** (user, 2026-08-27) — `/cycle-issues --spawn 3` → batch F (#12, #13, #14) per the [epic](https://github.com/parkrw/vanillin/issues/11); console kit batches B–E (94–104) follow phase 3.
...
## Handoff — batch A merged (84, 92, 93); phase 3 issues next, then batches B–E (console kit 94–104)

**Status:** BATCH A MERGED; head follow-ups built on `feat/console-glow`  **Branch:** `feat/console-glow`  **PR:** none  **Updated:** 2026-08-27

- **Merged 2026-08-27:** PRs #8 (84), #10 (92), #9 (93) and the docs branch, no conflicts. Head follow-ups (`CopyField secret` in `settings-panel.jsx`, live dots ringed across the panels) built on `feat/console-glow` — see the adjustments log. Glow timing left at the 2s default.
- **Worktrees:** the three batch-A worktrees were removed 2026-08-27 after each measured clean and fully pushed; the `cycle-a` window is closed. Branches stay until the PRs merge.
- **Batch record (spawned and landed 2026-08-27):** tmux window `cycle-a` (`@52`), one pane each, layout even-horizontal. 84 token-guard — `fix/token-guard`, `../vanillin-task84-token-guard`, pane `%146`, port 5201. 92 glow-pulse — `feat/glow-pulse`, `../vanillin-task92-glow-pulse`, pane `%147`, port 5202. 93 copy-field — `feat/copy-field`, `../vanillin-task93-copy-field`, pane `%148`, port 5203. All three branch from `bf986f8f9d7e` on the docs branch, so each carries the task files; the user merges each into `main` (the docs commit comes along once). Every worker reported; every report was reviewed (delegated diff read, `Owns` assertion, verify re-run in the worktree) and passed first time. Reports in `docs/TODO/reports/task{84,92,93}.md` (untracked scratch).
...
- **Suite baseline:** 837/837 on 2026-08-27 (`feat/console-glow` at the glow-reshape commit, idle machine, `node tests/run.mjs`, exit 0), no failing tests. `main` at `e7ea7451c49c` implies **834** (the reshape adds 3 shape tests); the 832 predicted after batch A was one low. Re-measure before every spawn and name the failing tests — a bare count is not evidence.
...
| 92  | glow-pulse               | ~S  | [x]    | complete 2026-08-27 on `feat/glow-pulse`, **PR #10**, review PASS; indeterminate now sweeps a 40% indicator (it painted nothing); `<Progress glow>` halo sits on the track root, not the indicator — `overflow: hidden` clips descendant shadows, user-approved deviation; `--glow-duration`/`--glow-strength` inherited by badge, status-dot and progress; head re-run 821/821 |
| 93  | copy-field               | ~S  | [x]    | complete 2026-08-27 on `feat/copy-field`, **PR #9**, review PASS; `ui/copy-field` — one-click copy with clipboard fallback, middle/end truncation, `secret` mask + reveal, polite live region; composes `ui/button`; 10 tests, head re-run 821/821 |
...
- **2026-08-27 — glow motion reshaped to ck-console's; colours unchanged.** The user asked why the console's glow did not look like ck-console's. A first attempt read the difference as hue and added `--glow-accent` (orange peaks, red alarms) — **rejected: keep the colours from the 08-25 brief ("all same color of the central dot"), change the animation.** The real difference is shape and tempo (`ck-console/packages/ui/console/console.css:797-814`): ck's halo is a crisp ring that swells, not a blurred glow that brightens, and its alarms run a fast 1.1s beat where the ring swells further and fades while the dot dims to 0.65. Now in the kit: `status-dot--ring` swells 3px → 5px with no blur; `badge--glow` and `progress--glow` swell 2px → 4px; `error` rings and destructive glow badges run `status-dot-alarm` / `badge-alarm` on `--glow-alarm-duration` (default 1.1s, the third shared property; 3px → 6px and 2px → 5px, alpha 30% → 12%, the dot to 0.65). `.ck-console` sets `--glow-duration: 2.4s`, ck's live tempo. The parked (reduced-motion) halo stays the shared 20% rest ring for every status. **Test method worth keeping:** a paused CSS animation plus a negative `animation-delay` lands a hair off the keyframe (rest L read 0.550014 for 0.55); the frame-sampling tests set `currentTime` through `el.getAnimations()`, which is exact, and split Chrome's `<colour> x y blur spread` serialisation at the first `)`. The rejected accent commit was reverted on the branch and the branch squashed to one commit.

- **2026-08-27 — head follow-ups from batch A landed on `feat/console-glow`.** The console mock's API-key row is now `CopyField secret` (`site/showcase/panels/settings-panel.jsx`), and every live signal in the panels rings: all eight status-widget dots (the panel's own static halo went, the ring paints it in both motion modes), the org footer's operational dot, the open incident's title dot, and the escalated tickets — open, pending and resolved rows sit still, since `ring` is the running/alarm look, not decoration. The console's own file already ringed its health rows, running tasks and live card. `--glow-duration`/`--glow-strength` were left at defaults; retime from a running console, not a still. Three tests added in `tests/showcase-panels.test.mjs` (escalated-only ring, CopyField masks/reveals/copies the real key, widget dots animate and keep a static halo under reduced motion).

- **2026-08-27 — batch A landed: 84 + 92 + 93, all three PASS on first review.** PRs #8 (84), #10 (92), #9 (93), all pushed by the workers with the user's approval in-pane. **First batch where every worker's reported count matched the supervisor's re-run** — 812, 821, 821 against the 811 baseline, zero `FAIL` lines in any of the six runs; the "name the failing tests" seed line and the 811 baseline in every task file are what changed. Three things worth keeping. **(1) Two spec assertions were wrong and a worker caught each by measuring first.** 92's `.progress-indicator` halo could not paint — `.progress { overflow: hidden }` clips descendant shadows, while `getComputedStyle().boxShadow` still reads non-`none`, so the specified test would have passed over an invisible halo (the `mask-image` failure mode AGENTS.md records); the halo moved to the track root with the user's approval. And `getBoundingClientRect().width > 0` cannot detect the indeterminate bug because the rect ignores `translateX` — the test now asserts track overlap and frame-sampled motion. Both went to `docs/QUIRKS.md`. **(2) 84's spec premise was half false**: Chrome drops `@property --typeset-size` and `--typeset-flow` because a `rem` initial value is not computationally independent, so only `--typeset-leading` was ever guarded — filed as **C17**, outside 84's scope. The JS-set token count is 19, not 17; the two extras are consumer-override hooks read with a fallback, so "a `var()` with a fallback is never a finding" became a second rule. H4 is closed. **(3) The `--spawn` monitor snippet is bash; the Monitor shell here is zsh**, which does not word-split `$panes`, so the first sweep reported `PANE GONE task84` twenty seconds after spawn with every pane alive. Re-armed with a literal word list; recorded in memory. Worktrees branched from the docs branch tip (`bf986f8f9d7e`) rather than `main` so they carried the task files without a merge first — worked, no conflicts, and the docs commit rides in with whichever PR merges first.
