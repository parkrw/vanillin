...
## Where things stand (session 4 of 2026-07-26)

**Phase 2 is integrated and pushed.** `main`, `feat/phase-2` and `origin/main`
are all at `c7b552089c76`. Suite **560/560**, `npm run build` clean. The working
tree is clean; nothing is stashed. `feat/phase-2` is now redundant — it points at
the same commit as `main` and can be deleted whenever.

**All 17 stale worktrees and their branches are gone.** Verified first that every
branch was fully absorbed: 16 of 17 had every commit already in `main` by
patch-id, and the 17th (`fix/phase-2-integration`) contributed only 3 commits
that would each have regressed the tree — it emptied the config to `{}`,
reverted the generator-header assertion to `vanillin`, and added an
`about:blank` workaround that `resetPage()` supersedes. Nothing was lost.

### What landed this session

- **The `van` rename is committed** (`0f7c80a24013`). `van.config.json`,
  `styles/van.css`, `/* Generated by van */`. Deliberately *not* renamed: the
  package name, the `vanillin:resizable:*` localStorage key (consumers have
  saved panel state and two tests assert the literal), the `vanillin-search`
  highlight-registry key, and playground prose.
- **Task 63.2 done** (`9418abc2541d`) — see Decisions below.
- **shadcn is unnamed everywhere except the introduction page**
  (`4f64675b5fdc`, `c7b552089c76`). 34 mentions in the shipped surface and 82 in
  the planning docs. Where a comparison is load-bearing the text says
  "upstream" — including the Conventions bullet below, which now reads
  "upstream's exact export names" and means the same constraint.
...
**Batch 49/61/62 is integrated on `feat/batch-49-61-62`** (16 commits: 15
cherry-picked with zero conflicts + one head fix). Suite **583/583**,
`npm run build` clean, no new deps. The head fix: 62's Schema docs page made
four docs-nav entries where `docs-shell.test.mjs` asserted three — invisible
to per-file worker runs, caught by the integrated suite, as designed.

**User runs the merge:**

```
git switch main && git merge --ff-only feat/batch-49-61-62 && git push origin main
```

After the merge: delete `feat/data-table-scale`, `feat/brand-multicolor`,
`feat/schema-core` (verify with `git cherry main <branch>`, then `-D` — the
worker branches lack the docs-shell fix so ancestry checks will lie, as
always), remove the three worktrees, and kill tmux windows `vanillin:4–6`.
`feat/phase-2` and `feat/batch-49-61-62` also become deletable.

Then, in order:

1. **64** serially next (wide footprint), then **38** — `add` consumes the
   manifest format.
2. **60** (61 landed, now unblocked), **63** remainder (49 landed, now
   unblocked), **39** (overlaps all component CSS — run alone), **65**
   (blocked on 64+38).
3. **30** docs pass last.

Task files exist for every task except 22–29 and 59.
